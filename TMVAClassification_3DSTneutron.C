...
int TMVAClassification_3DSTneutron( TString myMethodList = "" )
{
   ...
   TTree *signalTree     = (TTree*)input->Get("output_tree");
   TTree *background     = (TTree*)input->Get("output_tree");

   //TTree *signalTreeTest     = (TTree*)inputTest->Get("h1");
   //TTree *backgroundTest     = (TTree*)inputTest2->Get("h1");

   // Create a ROOT output file where TMVA will store ntuples, histograms, etc.
   TString outfileName( "TMVA_3DSTneutron.root" );
  ...
  // dataloader->AddSpectator( "tofSmear:=tofSmear",  "tofSmear", "units", 'F' );
   ...
   if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=850:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20" ); //(GiniIndex) SDivSqrtSPlusB:nCuts=20" );
...
int main( int argc, char** argv )
{
   ...
   return TMVAClassification_3DSTneutron(methodList);
}
